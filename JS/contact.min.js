// Your web app's Firebase configuration
var firebaseConfig = {
  apiKey: "AIzaSyANuNA-Re30sMGKAaNJUiBTBwwgwhNy6RY",
  authDomain: "contact-fc690.firebaseapp.com",
  projectId: "contact-fc690",
  storageBucket: "contact-fc690.appspot.com",
  messagingSenderId: "732499686368",
  appId: "1:732499686368:web:aba85abd4833fa8c55f185",
  databaseURL:
    "https://contact-fc690-default-rtdb.asia-southeast1.firebasedatabase.app/",
};
// Initialize Firebase
firebase.initializeApp(firebaseConfig);

//   Reference contactInfo collections
let contactInfo = firebase.database().ref("infos");

// Listen for a sumbmit

document.querySelector(".single-form").addEventListener("submit", submitForm);

function submitForm(e) {
  e.preventDefault();

<<<<<<< HEAD
  // Get input values
  let from_name = document.querySelector(".contact-name").value;
  let reply_to = document.querySelector(".contact-subject").value;
  let message = document.querySelector(".contact-message").value;
=======
    // Get input values
    let from_name = document.querySelector(".contact-name").value;
    let reply_to = document.querySelector(".contact-subject").value;
    let message= document.querySelector(".contact-message").value;
>>>>>>> 0b567e5e378cb7163aa6f0fc56b98c28220eaf22

  // console.log(name, email, message)

<<<<<<< HEAD
  saveContactInfo(from_name, reply_to, message);
=======
    saveContactInfo(from_name, reply_to, message);
>>>>>>> 0b567e5e378cb7163aa6f0fc56b98c28220eaf22

  document.querySelector("#contactForm").reset();
  // window.location.replace("index.html");

  // sendEmail(name, email, message);
}

// Save Infos to firebase
function saveContactInfo(from_name, reply_to, message) {
<<<<<<< HEAD
  let newContactInfo = contactInfo.push();

  newContactInfo.set({
    name: from_name,
    email: reply_to,
    message: message,
  });
=======
    let newContactInfo = contactInfo.push()

    newContactInfo.set({
        name: from_name,
        email: reply_to,
        message: message,
    });
>>>>>>> 0b567e5e378cb7163aa6f0fc56b98c28220eaf22
}

// // Send Email Info
// function sendEmail(name, email, message) {
//     Email.send({
//         Host: "smtp.gmail.com",
//         Username: 'sanjayvihaan111@gmail.com',
//         Password: "dmpbeiteznkiqobm",
//         To: 'sanjayvihaan111@gmail.com',
//         From: `${email}`,
//         Subject: `${name} sent you a message`,
//         Body: `Name 👨‍⚖️ : ${name} <br/> Email 📧 : ${email} <br/> Message: ${message}`,
//     }).then((_message) => alert("Thank you contacting, I will get back soon"))
// }

function sendMail(parmas) {
<<<<<<< HEAD
  var templ = {
    to_name: "sanjay",
    from_name: document.getElementById("from_name").value,
    message: document.getElementById("message").value,
    reply_to: document.getElementById("reply_to").value,

    // to_name: 'V Sanjay',
    // from_name : document.getElementById("from_name").value,
    // email_id : document.getElementById("reply_to").value,
    // message: document.getElementById("message").value,
  };

  emailjs.send("service_aghs6sk", "template_ogace3f").then(function (res) {
    alert("Thank you contacting, I will get back soon", res.status);
  });
}

/*
emailjs.send("service_aghs6sk","template_ogace3f",{
to_name: "sanjay",
from_name: "venkatesh",
message: "Hello my venkates",
reply_to: "venkatesh.venki769@gmail.com",
});
*/
=======
    var templ = {
        to_name: 'V Sanjay',
        from_name : document.getElementById("from_name").value,
        email_id : document.getElementById("reply_to").value,
        message: document.getElementById("message").value,
    };

    emailjs.send('default_service','template_ogace3f',templ)
    .then(function(res){
        alert("Thank you contacting, I will get back soon", res.status)
    })
}
>>>>>>> 0b567e5e378cb7163aa6f0fc56b98c28220eaf22
